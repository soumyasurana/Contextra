...
struct UnconfiguredGatewayService;
...
mod tests {
    ...
    use providers::{ChatMessage, ChatRole};
    ...
    #[tokio::test]
    async fn list_documents_uses_auth_request_id_and_cursor_pagination()
    -> Result<(), Box<dyn std::error::Error>> {
        let service = Arc::new(MockGatewayService::default());
        ...
        assert!(service.auth_calls.load(Ordering::SeqCst) > 0);
        Ok(())
    }

    #[tokio::test]
    async fn execute_chat_calls_gateway_service() -> Result<(), Box<dyn std::error::Error>> {
        let service = Arc::new(MockGatewayService::default());
        let app = test_router(service.clone());
        ...
        let body: Value =
            serde_json::from_slice(&to_bytes(response.into_body(), usize::MAX).await?)?;
        assert_eq!(body["message"], "mock answer");
        assert_eq!(service.chat_calls.load(Ordering::SeqCst), 1);
        Ok(())
    }

    #[tokio::test]
    async fn stream_chat_returns_sse_event() -> Result<(), Box<dyn std::error::Error>> {
        let app = test_router(Arc::new(MockGatewayService::default()));
        let conversation_id = ConversationId::new();
        ...
        assert!(body.contains("mock answer"));
        Ok(())
    }

    #[tokio::test]
    async fn missing_auth_returns_structured_error() -> Result<(), Box<dyn std::error::Error>> {
        let app = test_router(Arc::new(MockGatewayService::default()));
    ...
    #[tokio::test]
    async fn rate_limit_returns_429() -> Result<(), Box<dyn std::error::Error>> {
        let state = AppState::new(Arc::new(MockGatewayService::default()))
            .with_rate_limiter(TokenBucketRateLimiter::new(1, 1, Duration::from_secs(60)));
            ...
            let response = app
                ...
                .oneshot(
                    Request::builder()
                        ...
                        .header(header::AUTHORIZATION, "Bearer rate-token")
                        .body(Body::empty())?,
                )
                .await?;
    ...
    fn test_router(service: Arc<dyn GatewayService>) -> Router {
    ...
    impl GatewayService for MockGatewayService {
        async fn authenticate(&self, token: &str) -> Result<AuthContext, ContextraError> {
            ...
            if token == "test-token" || token == "rate-token" {
                Ok(AuthContext::new(
                    UserId::new(),
                    OrgId::new(),
                    vec!["*:*".to_string()],
                ...
                Err(ContextraError::Unauthorized("invalid token".to_string()))
            }
        }
            ...
            Ok(page(
                vec![
                    DocumentResource {
                        ...
                        content: "doc one".to_string(),
                        metadata: Metadata::new(),
                    },
                    DocumentResource {
                        id: DocumentId::new().to_string(),
                        collection_id: CollectionId::new().to_string(),
                        content: "doc two".to_string(),
                        metadata: Metadata::new(),
                    },
                ],
                pagination,
        ...
        async fn execute_chat(
            ...
            Ok(ChatExecutionResponse::from(ChatResponse {
    ...
    fn page<T: Clone>(items: Vec<T>, pagination: Pagination) -> Page<T> {
        let start = pagination.cursor.unwrap_or(0);
        let end = (start + pagination.limit).min(items.len());
        let page_items = items[start..end].to_vec();
        let has_more = end < items.len();
        Page::new(
            page_items,
            has_more.then(|| Cursor(end.to_string())),
            has_more,
